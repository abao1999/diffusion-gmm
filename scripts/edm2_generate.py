        ...
        if i % snapshot_interval == 0 or i == num_steps - 1:
            if snapshot_save_dir:
                for snapshot_name, snapshot_images in zip(
                    ...
                    save_seeds = random_seeds[:n_images_save]
...
def main(cfg: DictConfig):
    ...
    """
    edm2_cfg = OmegaConf.to_container(cfg.edm2, resolve=True)
    opts = dnnlib.EasyDict(edm2_cfg)  # type: ignore
    seeds = opts["seeds"]
    opts["seeds"] = parse_int_list(seeds)

    # Apply preset.
    if opts.preset is not None:
        if opts.preset not in config_presets:
            raise ValueError(f'Invalid configuration preset "{opts.preset}"')
    ...
    # Validate options.
    if opts.net is None:
        raise ValueError("Please specify either --preset or --net")
    if opts.guidance is None or opts.guidance == 1:
        opts.guidance = 1
        opts.gnet = None
    elif opts.gnet is None:
        raise ValueError("Please specify --gnet when using guidance")
