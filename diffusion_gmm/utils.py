...
def save_and_plot_samples(
    ...
    save_grid_shape: Optional[Tuple[int, int]] = (10, 10),
    ...
    if process_fn is not None:
        ...
        assert samples.dtype == np.uint8, "Samples must be converted to np.uint8 before saving as image."
        ...
        warnings.warn("No image post-processing function provided. Saving samples as numpy files. This may not be the desired behavior.")
...
def default_image_processing_fn(samples: np.ndarray, verbose: bool = True) -> np.ndarray:
    """
